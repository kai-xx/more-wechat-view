<template>
  <div class="app-container">
    <el-table :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column class-name="status-col" label="粉丝编号">
        <template slot-scope="scope">
          <span>{{ scope.row.fans_id }}</span>
        </template>
      </el-table-column>

      <el-table-column class-name="status-col" label="昵称" width="80">
        <template slot-scope="scope">
          <el-tag >{{ scope.row.nick}}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="status-col" label="状态">
        <template slot-scope="scope">
          <el-tag >{{ scope.row.status | logTypeFilter}}</el-tag>
        </template>
      </el-table-column>
